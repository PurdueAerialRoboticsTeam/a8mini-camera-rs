...
pub const API_BASE_URL: &str = "//cgi-bin/media.cgi";
...
// +-----------+-------+---------+---------------------------------------------------+
// | CTRL      | 2     | 1       | 0: need_ack (if the current data pack needs ack)  |
// |           |       |         | 1: ack_pack (if the current data pack is an ack)  |
// |           |       |         | 2-7: reserved                                     |
// +-----------+-------+---------+---------------------------------------------------+
// | DATALEN   | 3     | 2       | Data field byte length                            |
// |           |       |         | Low byte in the front                             |
// +-----------+-------+---------+---------------------------------------------------+
...
//
//     +-----------+-----+-----------+-----------+-----+------- ... --+-----------+
//     |    STX    |CTRL |  DATALEN  |    SEQ    | CMD |  DATA  ...   |   CRC16   |
//     +-----------+-----+-----------+-----------+-----+------- ... --+-----------+
pub const NUM_COMMANDS: usize = 31; // update this if more commands are added
pub const HARDCODED_COMMANDS: [&[u8]; NUM_COMMANDS] = [
    &[
        0x55, 0x66, 0x01, 0x01, 0x00, 0x00, 0x00, 0x08, 0x01, 0xd1, 0x12,
    ], // Auto Center
    &[
        0x55, 0x66, 0x01, 0x02, 0x00, 0x00, 0x00, 0x07, 0x00, 0x2D, 0x3e, 0xd1,
    ], // Rotate Up
    &[ 
        0x55, 0x66, 0x01, 0x02, 0x00, 0x00, 0x00, 0x07, 0x00, 0u8.wrapping_sub(0x2D), 0xef, 0xdf,
    ], // Rotate Down
    &[ 
        0x55, 0x66, 0x01, 0x02, 0x00, 0x00, 0x00, 0x07, 0u8.wrapping_sub(0x2D), 0x00, 0x85, 0x64,
    ], // Rotate Right
    &[
        0x55, 0x66, 0x01, 0x02, 0x00, 0x00, 0x00, 0x07, 0x2D, 0x00, 0x4b, 0x54,
    ], // Rotate Left
    &[
        0x55, 0x66, 0x01, 0x02, 0x00, 0x00, 0x00, 0x07, 0x00, 0x00, 0xf1, 0x24,
    ], // Stop Rotation
    &[
    ...
    &[
        0x55, 0x66, 0x01, 0x01, 0x00, 0x00, 0x00, 0x04, 0x01, 0xbc, 0x57,
    ], // Auto Focus
    &[
    ...
    &[
        0x55, 0x66, 0x01, 0x01, 0x00, 0x00, 0x00, 0x0c, 0x06, 0xf2, 0xae,
    ], // Set Video Output as HDMI
    &[
        0x55, 0x66, 0x01, 0x01, 0x00, 0x00, 0x00, 0x0c, 0x07, 0xd3, 0xbe,
    ], // Set Video Output as CVBS
    &[
        0x55, 0x66, 0x01, 0x01, 0x00, 0x00, 0x00, 0x0c, 0x08, 0x3c, 0x4f,
    ], // Turn Off both CVBS and HDMI Output
    &[
        0x55, 0x66, 0x01, 0x00, 0x00, 0x00, 0x00, 0x15, 0xD1, 0x96
    ], // Read Range from Laser Rangefinder
    &[
        0x55, 0x66, 0x01, 0x02, 0x00, 0x00, 0x00, 0x80, 0x01, 0x00, 0x0a, 0xa9,
    ], // Reboot Camera
    &[
        0x55, 0x66, 0x01, 0x02, 0x00, 0x00, 0x00, 0x80, 0x00, 0x01, 0x1a, 0x8a,
    ], // Reboot Gimbal
    &[
        0x55, 0x66, 0x01, 0x09, 0x00, 0x00, 0x00, 0x21, 0x00, 0x02, 0x00, 0x0f, 0x70, 0x08, 0x98, 0x3a, 0x00, 0x70, 0xbe,
    ], // Resolution 4k
    &[
